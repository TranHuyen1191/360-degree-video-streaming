...
fname = sprintf('Log%sFrance_Cube_log_%.0fface%.0fx%.0f_vbr_theta.txt',bracket,No_face,tile_hori_num,tile_ver_num);
fout = fopen(fname,'w');

fname_err = sprintf('Log%sFrance_Cube_log_%.0fface%.0fx%.0f_vbr_theta_Err.txt',bracket,No_face,tile_hori_num,tile_ver_num);
...
for i = 1:1:1 %% Original 1:1:11
    ...
    for angle_phi = 0:30:0 
        ...
        for angle_theta = -165:15:-165   %% Original -165:15:180 
            ...
            %%%% ERROR %%%%%
            fprintf(fout_err,'%.0f\t%.0f\t%.0f\t',Bw,angle_phi,angle_theta_name);
            for Err_idx = 1:1:7
                Err = (Err_idx-1)*10;
                % Calculate phi_err and theta_err
                phi_err_pi = phi;
                theta_err = angle_theta + Err;
                if(theta_err >=180)
                    theta_err = theta_err - 360;
                end
                theta_err_pi = theta_err/180*pi;
