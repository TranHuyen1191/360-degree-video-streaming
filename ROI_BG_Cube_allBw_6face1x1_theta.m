...
fname = sprintf('Log%sROI_BG_Cube_log_%.0fface%.0fx%.0f_vbr_theta.txt',bracket,No_face,tile_hori_num,tile_ver_num);
fout = fopen(fname,'w');
...
for i = 1:1:2  %% Original 1:1:11
    ...
    for angle_phi = 0:30:0 
		...
		for angle_theta = -165:15:-165  %% Original -165:15:180
			...
			[S_t,T,sel_BR] = F_ROI_BG_Cube(angle_phi,Fh, Fv, vp_W,vp_H, face_W, face_H, phi,theta,No_face,tile_hori_num,tile_ver_num,No_ver,Bw,BR,deltaBR,PSNR,MSE,deltaMSE,LB_tile_W,LB_tile_H,HB_tile_W,HB_tile_H);
			fprintf(fout,'%.0f\t%.0f\t%.0f\t',Bw,angle_phi,angle_theta_name);
			for i = 1: No_face
				for k = 1: No_tile
					fprintf(fout,'%.0f\t',T(i,k));
				end
			end

			fprintf(fout,'%.0f\t',sel_BR);
			for i = 1: No_face
				for k = 1: No_tile
					fprintf(fout,'%.0f\t',S_t(i,k));
				end
			end
			fprintf(fout,'\n');
		end
